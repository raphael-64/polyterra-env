...
def main():
    # Training config - all tracked in W&B
    config = {
    ...
    # Initialize W&B (sync_tensorboard=True requires tensorboard package)
    run = wandb.init(
        project="polyterra",
        config=config,
        save_code=True,
    ...
    # Create model (verbose=0 to suppress terminal output)
    model = MaskablePPO(
        config["policy"],
        env,
        verbose=0,
    ...
    # Quick test episode
    test_env = PolyterraGymWrapper(num_players=2, max_steps=100)
