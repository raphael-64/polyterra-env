...
public class UpgradeEmbassyCommand : CommandBase
{
	public WorldCoordinates Coordinates { get; protected set; }

	public byte OpponentId { get; protected set; }

	public UpgradeEmbassyCommand()
	{
	}

	public UpgradeEmbassyCommand(byte playerId, byte opponentId, WorldCoordinates coordinates)
		: base(playerId)
	{
	...
	public override bool IsValid(GameState gameState, out string validationError)
	{
		...
		if (playerState.Currency < gameState.GameLogicData.DiplomacyData.embassyUpgradeCost)
		{
		...
		int embassyMaxLevel = gameState.GameLogicData.DiplomacyData.embassyMaxLevel;
		if (!playerState.HasEmbassyWith(playerState2) && playerState.GetEmbassyLevel(playerState2) < embassyMaxLevel)
		{
	...
	{
		state.ActionStack.Add(new UpgradeEmbassyAction(base.PlayerId, OpponentId));
	}

	public override CommandType GetCommandType()
	{
		return CommandType.UpgradeEmbassy;
	}
