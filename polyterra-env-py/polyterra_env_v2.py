...
class PolyterraEnvV2(AECEnv):
    ...
    def _render_ansi(self):
        ...
        output = []
        output.append("=" * 70)
        output.append(f"POLYTOPIA V2 - Turn {obs.get('turn', 0)}")
        output.append(f"Player: {current_agent} | Currency: {obs.get('currency', 0)} | Score: {obs.get('score', 0)}")
        output.append(f"Cities: {obs.get('num_cities', 0)} | Kills: {obs.get('num_kills', 0)}")
        output.append("=" * 70)
